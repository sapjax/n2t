/**
 * This file is from https://github.com/nickewing/line-reader,
 * and the reason why it is not installed by npm is that the test environment of the course does not support npm
 */
...
function createLineReader(readStream, options, creationCb) {
  ...
  if (separator instanceof RegExp) {
    findSeparator = function () {
      ...
      if (
        result &&
        (result.index + result[0].length < bufferStr.length || eof)
      ) {
...
function open(filenameOrStream, options, cb) {
  ...
  if (typeof filenameOrStream.read == 'function') {
    ...
    cb(
      new Error(
        'Invalid file argument for LineReader.open.  Must be filename or stream.'
      )
    )
    return
  }
